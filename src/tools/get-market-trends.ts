...
export function getMarketTrendsTool(server: McpServer) {
  server.tool(
    "getMarketTrends",
    {
...
function determineMarketSentiment(metrics: any): string {
  let score = 0;
  
  // Volume change impact
  if (metrics.volume_change_24h > 20) score += 2;
  else if (metrics.volume_change_24h > 0) score += 1;
  else if (metrics.volume_change_24h < -20) score -= 2;
  else if (metrics.volume_change_24h < 0) score -= 1;
  
  // Sales change impact
  if (metrics.sales_change_24h > 15) score += 1;
  else if (metrics.sales_change_24h < -15) score -= 1;
  
  // Active wallets change
  if (metrics.active_wallets_change > 10) score += 1;
  else if (metrics.active_wallets_change < -10) score -= 1;
  
  // Price change impact
  if (metrics.average_price_change > 10) score += 1;
  else if (metrics.average_price_change < -10) score -= 1;
...
function generateMarketInsights(metrics: any, trending: any[], sentiment: string): string[] {
  const insights: string[] = [];
  
  // Volume insights
  if (Math.abs(metrics.volume_change_24h) > 30) {
    insights.push(
      metrics.volume_change_24h > 0
  ...
  if (Math.abs(metrics.active_wallets_change) > 20) {
    insights.push(
      metrics.active_wallets_change > 0
...
function generateMarketAnalysis(metrics: any, sentiment: string): string {
...
function identifyOpportunities(trending: any[], metrics: any): string[] {
  const opportunities: string[] = [];
  
  // Look for undervalued collections
  const undervalued = trending.filter(c => 
    c.volume_change_24h > 20 && 
  ...
  // High holder growth
  const growingCommunity = trending.filter(c => 
    c.owner_count > 2000 && 
