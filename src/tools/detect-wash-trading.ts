...
export function detectWashTradingTool(server: McpServer) {
  server.tool(
    "detectWashTrading",
    {
    ...
    async ({ collection_address, time_range = "7d", blockchain = "ethereum" }) => {
      ...
      try {
        ...
        const analysis = generateWashTradingAnalysis(
          washTradingData,
          collectionData,
          severity
        );
...
function getWashTradingSeverity(percentage: number): string {
...
function generateWashTradingAnalysis(
  washTradingData: any,
  collectionData: any,
  severity: string
): string {
...
function getRecommendations(severity: string, percentage: number): string[] {
