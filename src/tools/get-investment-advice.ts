...
export function getInvestmentAdviceTool(server: McpServer) {
  server.tool(
    "getInvestmentAdvice",
    {
    ...
    async ({ budget, risk_tolerance, investment_horizon, categories = [] }) => {
      ...
      try {
        ...
        const scoredCollections = await scoreCollections(
          trendingCollections,
          risk_tolerance,
          investment_horizon,
          budget,
        ...
        const result = {
          ...
          investment_strategy: generateInvestmentStrategy(
            risk_tolerance,
            investment_horizon,
            marketCondition
          ),
          warnings,
          diversification_advice: generateDiversificationAdvice(
            recommendations,
            budget
          ),
          exit_strategy: generateExitStrategy(
            risk_tolerance,
            investment_horizon
          )
        };
...
function analyzeMarketCondition(metrics: any): any {
  const sentiment = 
    metrics.volume_change_24h > 10 && metrics.sales_change_24h > 5 ? "bullish" :
...
function generateMarketDescription(sentiment: string, metrics: any): string {
...
async function scoreCollections(
  collections: any[],
  riskTolerance: string,
  horizon: string,
  budget: number,
  client: UnleashNFTsClient
): Promise<any[]> {
  const scored = [];
  
  for (const collection of collections) {
    // Skip if floor price exceeds budget
    if (collection.floor_price > budget) continue;
...
function generatePortfolioRecommendations(
  ...
  const allocationStrategy = 
    riskTolerance === 'conservative' ? [0.4, 0.3, 0.2, 0.1] :
...
function generateRecommendationReasoning(collection: any, riskTolerance: string, horizon: string): string {
...
function getRiskLevel(collection: any, riskTolerance: string): string {
...
function generateExpectedReturn(collection: any, horizon: string): string {
  if (horizon === 'short') {
...
function calculatePortfolioMetrics(recommendations: any[], riskTolerance: string): any {
...
function generateWarnings(marketCondition: any, riskTolerance: string, budget: number): string[] {
  const warnings = [];
...
function generateInvestmentStrategy(riskTolerance: string, horizon: string, marketCondition: any): string {
  let strategy = "";
...
function generateDiversificationAdvice(recommendations: any[], budget: number): string {
...
function generateExitStrategy(riskTolerance: string, horizon: string): string {
  if (horizon === 'short') {
