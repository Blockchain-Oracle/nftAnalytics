...
import { UnleashNFTsClient, CollectionData } from "./unleash-client.js";
...
server.addTool({
  ...
  execute: async ({ collection_address, blockchain }: { collection_address: string; blockchain?: string }, { log }: { log: any }) => {
...
server.addTool({
  ...
  execute: async ({ time_range, include_trending }: { time_range?: string; include_trending?: boolean }, { log }: { log: any }) => {
    const client = new UnleashNFTsClient();
    ...
    try {
      ...
      let trendingCollections: CollectionData[] = [];
      if (include_trending) {
...
server.addTool({
  ...
  execute: async ({ wallet_address, blockchain }: { wallet_address: string; blockchain?: string }, { log }: { log: any }) => {
    const client = new UnleashNFTsClient();
...
server.addTool({
  ...
  execute: async ({ collection_address, blockchain, investment_amount, risk_tolerance }: { collection_address: string; blockchain?: string; investment_amount?: number; risk_tolerance?: "low" | "medium" | "high" }, { log }: { log: any }) => {
    const client = new UnleashNFTsClient();
