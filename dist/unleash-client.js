...
export class UnleashNFTsClient {
    ...
    async getCollectionData(address, blockchain = 'ethereum') {
        try {
    ...
    async getMarketMetrics(timeRange = '24h') {
        try {
    ...
    async getWalletProfile(address, blockchain = 'ethereum') {
        try {
            const blockchainId = this.getBlockchainId(blockchain);
    ...
    async getCollectionWashTrading(address, blockchain = 'ethereum') {
        try {
            const blockchainId = this.getBlockchainId(blockchain);
            ...
            if (data.data_points && data.data_points.length > 0) {
                ...
                const latestData = data.data_points.find((point) => point.values &&
